...
double dydw(double cleanprice, double dirtyprice, double netwage, double time_endowment, double virtual_inc, double A_1, double A_2, double A_3, double B_1, double B_2, double B_3, double a, double b, double c) {
	...
	double dydwterm1a = ( ( B_3 * cleanprice ) - ( B_2 * dirtyprice ) ) * ( time_endowment - c );
	double dydwterm1 = ( 4 * A_2 * b * cleanprice ) - dydwterm1a;
	double dydwterm2a = ( b * B_3 ) - ( A_3 * ( time_endowment - c ) );
	double dydwterm2b = virtual_inc - ( a * cleanprice ) + ( b * dirtyprice ) + ( 2 * netwage * ( time_endowment - c ) );
	double dydwterm2 = ( 2 * cleanprice * dydwterm2a ) + ( B_2 * dydwterm2b );
	double dydwterm3 = ( - 2 * A_3 * dirtyprice * ( time_endowment - c ) ) - ( 4 * A_2 * b * netwage ) + ( B_3 * dydwterm2b );
	double dydwterm = ( B_2 * dydwterm1 ) + ( 2 * b * B_1 * B_1 * netwage ) - ( B_1 * dydwterm2 ) + ( 2 * A_1 * dydwterm3 );

	double dydw = ( ( den * dydwterm ) - ( dwterm * dyterm ) ) / ( den * den );
	return dydw;
}