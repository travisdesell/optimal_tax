...
double objective_function(const vector<double> &A, uint64_t iteration) {
    double f = 0;
    ...
    if (success_count >= 14 && f > -0.01) {
        (*output_csv) << iteration << ", " << A[0] << ", " << A[1] << ", " << A[2] << ", " << A[3] << ", " << A[4] << ", " << A[5] << ", " << success_count << ", " << f << ", " << mux_v << ", " << muy_v << ", " << mul_v << ", " << elastxp << ", " << elastxq << ", " << elastxw << ", " << elastyp << ", " << elastyq << ", " << elastyw << ", " << elastlp << ", " << elastlq << ", " << elastlw << ", " << etax << ", " << etay << ", " << etal << ", " << minor2_v << ", " << minor3_v << ", " << minor4_v << endl;
    }

    return f;
}

double objective_function2(const vector<double> &A) {
    double f = 0;
    uint32_t success_count = 0;
...
int main(int number_arguments, char **argv) {
    ...
    if (search_type.compare("ps") == 0) {
        ...
        if (search_type.compare("snm") == 0) {
            synchronous_newton_method(arguments, objective_function2, starting_point, step_size);
        } else if (search_type.compare("gd") == 0) {
            synchronous_gradient_descent(arguments, objective_function2, starting_point, step_size);
        } else if (search_type.compare("cgd") == 0) {
            synchronous_conjugate_gradient_descent(arguments, objective_function2, starting_point, step_size);
        }
