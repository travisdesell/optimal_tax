...
double cleanprice = 1.687446;
double dirtyprice = 1.07299;
double netwage = 7.487497;
double cleangood = 203.4541;
double dirtygood = 24.0152;
double leisuregood = 47.21569;
double time_endowment = 80;
double virtual_inc = 113.5404;
double total_income = 712.5401;
...
double objective_function(const vector<double> &A) {
    ...
    double elastyw = dydw(cleanprice, dirtyprice, netwage, time_endowment, virtual_inc, A_1, A_2, A_3, B_1, B_2, B_3, a, b, c) * ( netwage / dirtygood );           /* abs val ( elastyw ) < 1; */
     ...
     if (-0.50 < elastyp && elastyp < 0.00) success_count++; f -= penalty_out_inclusive(-1.00, 1.00, elastyp);
     if (-0.70 < elastyq && elastyq < -0.20) success_count++; f -= penalty_out_inclusive(-1.00, -0.20, elastyq);
     if (0.60 < elastyw && elastyw < 0.90) success_count++; f -= penalty_out_inclusive(-1.25, 1.25, elastyw);
    ...
    double elastlw = dldw(cleanprice, dirtyprice, netwage, time_endowment, virtual_inc, A_1, A_2, A_3, B_1, B_2, B_3, a, b, c) * ( netwage / leisuregood );         /* abs val ( elastlw ) < 1; */
     ...
     if (-0.15 < elastlw && elastlw < 0.15) success_count++; f -= penalty_out_inclusive(-0.25, 0.25, elastlw);
